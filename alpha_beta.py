...
def find_best_action_alpha_beta(self, player):
    ...
    for action in self.get_all_possible_actions(player):
        new_game = self.simulate_action(action)
        score, _ = new_game.alpha_beta_search(3, alpha, beta, player == 'attacker') 
        #_: ignore best_move, only store score
...
def ai_vs_ai():
    ...
    while not game.game_over():
        ...
        if game.player.name == 'attacker':
            ...
            game.clear()
            attacker_move = game.find_best_action_alpha_beta('attacker')
            game.perform_action(attacker_move)
            game.clear()
        else:
            print(game.player.name)
            game.clear()
            defender_move = game.find_best_action_alpha_beta('defender')
            game.perform_action(defender_move)
            game.clear()
        game.switch_turn()
